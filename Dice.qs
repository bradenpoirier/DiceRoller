...
namespace Microsoft.Quantum.Samples.DiceRoller {
    ...
    operation Roll () : (Int) {
        ...
        using (qubits = Qubit[3]) {
            ...
            // which puts them into a superposition state with 50% probability of collapsing
            // into a 0 or 1 upon measurement.
            H(qubits[0]); 
            H(qubits[1]); 
            H(qubits[2]); 
            ...
            // We could have applied RESET(qubits[0]) instead of M(qubits[0]) and we wouldn't have had 
            // to call X(qubits[n]) to flip the bits back to a zero whenever they are set to a one.
